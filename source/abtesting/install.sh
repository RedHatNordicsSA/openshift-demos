...
oc expose service php72 --name='ab-php' -l name='ab-php' -n demo-abtesting

echo "$(date): Configuring load balancing between the two applications."
oc set route-backends ab-php php73=10 php72=90 -n demo-abtesting
...
while true; do
	if oc get builds -n demo-abtesting|egrep '(php73|php72)'|grep Running|wc -l|grep 0 >/dev/null; then
		if oc get pods -n demo-abtesting|egrep '(php73|php72)'|grep Running|grep "1/1"|wc -l|grep 2 >/dev/null; then
