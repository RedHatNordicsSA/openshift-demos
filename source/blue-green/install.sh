...
oc new-project demo-greenblue --description "Demo of green-blue testing using PHP 7.3 and 7.2" --display-name "Demo - Green-Blue testing"

oc project demo-greenblue

echo "$(date): Creating an PHP 7.3 application."
oc new-app openshift/php:7.3~https://github.com/mglantz/ocp-php.git --name=php73
...
echo "$(date): Creating a route and pointing it at php 7.2 app"
oc expose service php72 --name='greenblue-php' -l name='greenblue-php'

echo "$(date): Waiting for the php applications to build and deploy. This may take a bit: "
while true; do
	if oc get builds|egrep '(php72|php73)'|grep Running|wc -l|grep 0 >/dev/null; then
